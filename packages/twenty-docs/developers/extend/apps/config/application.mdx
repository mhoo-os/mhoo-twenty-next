...
`AppPath` also contains routes owned by the Twenty product itself, including public account and legal pages. Those paths are not app routes: apps should link to their own declared front components or logic-function routes instead of depending on product-owned paths.
