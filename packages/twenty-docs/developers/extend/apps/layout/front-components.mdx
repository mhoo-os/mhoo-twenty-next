...
The client configuration GraphQL types are generated from the server contract. They include brand metadata such as the approved legal-document URLs used by the product shell. Treat that metadata as read-only configuration: front components must not replace the product's public legal routes or invent document availability.
...
| `uploadFile(file, { fieldMetadataId, fileName? })` | Upload a `Blob` into a FILES field; resolves to `{ status: 'uploaded', file: { fileId, path, url, size, mimeType } }` or `{ status: 'failed', reason: 'invalid-params' \| 'upload-failed' }` |
