...
- live Plaid pending/posted/removed/replacement ingestion and nullable
  counterparty evidence (the lifecycle reducer and additive fact fields are
  source-complete only);
- live Clover tender-to-batch-to-funding inputs (the exact funding bridge and
  missing-component behavior are source-complete only);
...
bounded domain reducers. It does not yet prove installed event persistence,
native Task creation/attachments, provider populations, mailbox correlation or
