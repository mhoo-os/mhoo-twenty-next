...
export function FinanceInsights({data,isSynthetic,onOpenFollowUp,onOpenFact}:Props) {
  ...
  return <InsightsFrame><section className="hi" aria-label="Hass Kitchen Insights">
    ...
    {domainStart && domainEnd ? <FinancePeriodControls accounts={data.accounts} accountId={account} range={range} draft={draft} domainStart={domainStart} domainEnd={domainEnd} months={timelineMonths.map((month) => ({ ...month, start: dayDate(month.start), end: dayDate(month.end) }))} filtersOpen={filtersOpen} datesOpen={datesOpen} compare={compare} error={error} onAccountIdChange={(next) => { setAccount(next); setDrillMonth(null); }} onDraftChange={setDraft} onRangeChange={(next) => { if (!next.start || !next.end || next.start > next.end || next.start < domainStart || next.end > domainEnd) { setError(`Choose a valid range within ${domainStart} and ${domainEnd}.`); return; } setRange(next); setDraft(next); setError(''); setDrillMonth(null); setHover(null); }} onFiltersOpenChange={setFiltersOpen} onDatesOpenChange={setDatesOpen} onCompareChange={setCompare} /> : null}
