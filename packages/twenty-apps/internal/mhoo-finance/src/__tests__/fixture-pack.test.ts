...
describe('Mhoo Finance fixture-first vertical slice', () => {
  ...
  it('suppresses duplicate files, rows, and retries without changing authoritative facts', () => {
    ...
    expect(dataset.receipts.filter((receipt) => receipt.status === 'IMPORTED')).toHaveLength(11);
    expect(dataset.receipts.filter((receipt) => receipt.status === 'DUPLICATE')).toHaveLength(1);
  ...
  it('labels missing, zero-activity, stale, partial, and complete coverage separately', () => {
    const dataset = buildFixtureDataset();
    const coverageByKey = new Map(
      dataset.coverage.map((coverage) => [
        `${coverage.sourceKind}-${coverage.period}`,
        coverage,
      ]),
    );

    expect(coverageByKey.get('BANK-2026-05')).toMatchObject({
      status: 'NO_ACTIVITY',
      observedRows: 0,
      freshness: 'FRESH',
    });
    expect(coverageByKey.get('CLOVER-2026-04')).toMatchObject({
      status: 'NO_DATA',
      freshness: 'NOT_APPLICABLE',
    });
    expect(coverageByKey.get('CARD-2026-03')).toMatchObject({
      status: 'STALE',
      freshness: 'STALE',
    });
    expect(coverageByKey.get('TOAST-2026-02')).toMatchObject({
      status: 'PARTIAL',
      artifactIds: ['artifact-toast-2026-02'],
    });
    expect(coverageByKey.get('BANK-2026-01')).toMatchObject({
      status: 'PARTIAL',
      expectedPopulation: 'monthly statement',
    });
    expect(dataset.periods.every((period) => period.lineage.length > 0)).toBe(true);
  });

  it('keeps all artifact identity synthetic and redacted', () => {
    const dataset = buildFixtureDataset();

    expect(
      dataset.artifacts.every(
        (artifact) =>
          artifact.fileName.startsWith('synthetic-') &&
          artifact.contentHash.startsWith('fixture-'),
      ),
    ).toBe(true);
    expect(
      dataset.facts.every((fact) =>
        fact.description.toLowerCase().includes('synthetic'),
      ),
    ).toBe(true);
  });
