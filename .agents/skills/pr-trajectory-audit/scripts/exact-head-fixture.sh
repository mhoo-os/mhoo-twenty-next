...
git diff --name-only "$base" "$head" | while IFS= read -r path; do
  [[ "$path" =~ $allowed || "$path" =~ $email_allowed || "$path" =~ $distribution_allowed || "$path" =~ $scanner_allowed || "$path" =~ $legal_allowed || "$path" =~ $evaluation_allowed || "$path" =~ $loader_allowed || "$path" =~ $graphql_allowed || "$path" =~ $startup_allowed || "$path" =~ $ai_editor_allowed ]] || { printf 'trajectory fixture rejected: %s\n' "$path" >&2; exit 1; }
done
